...
Процедура ДобавитьЭлемент(Система, Архитектура, ИмяФайла)
	...
	Если Не Файл.Существует() Тогда
		Сообщить("ОШИБКА - Не найден файл: " + ИмяФайла);
	    Возврат;
	КонецЕсли;
...
Процедура MakePackage(ИмяПроекта)
	...
	Для НомерСтроки = 1 По 4 Цикл 
		...
		Если СтрНайти(Стр, "VERSION_FULL") > 0 Тогда
			...
			НомерВерсии = НаборСтрок.Получить(НаборСтрок.Количество() - 1);
	...
	ДобавитьЭлемент("Windows" , "i386"   , "bin32/Release/lib" + ИмяПроекта + "Win32.dll");
	ДобавитьЭлемент("Windows" , "x86_64" , "bin64/Release/lib" + ИмяПроекта + "Win64.dll");
	ДобавитьЭлемент("Linux"   , "i386"   , "bin32/lib" + ИмяПроекта + "Lin32.so");
	ДобавитьЭлемент("Linux"   , "x86_64" , "bin64/lib" + ИмяПроекта + "Lin64.so");
...
Если АргументыКоманднойСтроки.Количество() = 0 Тогда
	Сообщить("ОШИБКА - Не переданы параметры!");
Иначе
	Сообщить(АргументыКоманднойСтроки[0]);
	MakePackage(АргументыКоманднойСтроки[0]);
КонецЕсли;