&НаКлиенте
Перем ИдентификаторКомпоненты, ВнешняяКомпонента, ДанныеГрафа, ТекущийПуть;
...
Процедура ТекстHTMLДокументСформирован(Элемент)
	...
	Элементы.ТекстHTML.Документ.defaultView.setBasePath(ТекущийПуть);
...
Процедура ОбработкаОткрытияФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено Тогда
		ТекущийФайл = ВыбранныеФайлы[0];
		Файл = Новый Файл(ТекущийФайл);
		ТекущийПуть = Файл.Путь;
		ВнешняяКомпонента.НачатьУстановкуПутиКартинок(Новый ОписаниеОповещения, ТекущийПуть);
		ДанныеГрафа = Новый ДвоичныеДанные(ТекущийФайл);
		СформироватьКартинку();
	КонецЕсли;
